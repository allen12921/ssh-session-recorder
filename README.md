...
- `ssh -o ProxyCommand="ssh bastion nc %h %p"` 转发到第二台真实主机：转发功能本身正常（第二台主机上执行的命令能拿到正确输出），且确认审计日志里出现了准确的 `nc <目标 host> <目标 port>` 命令行和时间戳（细节见下面"已知取舍"一节）

> `session-shell` 里 sftp/rsync/scp 豁免分支的加固逻辑（属主校验、`realpath` 解析、TOCTOU 修复、命令行留痕）后来在一套真实环境里补做了完整的端到端验证：两个装了真实 `sshd`（OpenSSH 8.9p1/Ubuntu 22.04）的独立主机（不是同一台机器上自己连自己）+ 一台独立的 ssh 客户端主机，`/opt/audit` 和共享账号的 `~/.ssh` 都挂载在真正的 ext4 文件系统上（不是容器默认的 overlayfs——实测过 `chattr +a/+i` 在 overlayfs 上直接返回 `Operation not permitted`，必须是能记扩展属性的真文件系统，跟目标 Linux 主机一致），照着仓库自己的 `install.sh` 走完整安装流程，然后用真实的 `ssh`/`scp`/`sftp`/`rsync` 客户端（不是脚本模拟）把下面这些逐条跑了一遍：非交互命令+`-tt`交互命令+退出码透传、真实 `sftp`/`scp`/`rsync` 上传下载大文件（含二进制 md5 校验）确认零审计噪音、真实 `ssh -o ProxyCommand="ssh bastion nc %h %p"` 跳板转发到第二台主机（确认转发功能本身正常，同时确认日志里出现了准确的命令行+时间戳，且内层协议字节确实不可读——细节见下面"已知取舍"一节）、真实 `ssh -J` 确认零审计记录（验证了文档里说的硬限制是真的，不是没写代码）、真实构造的绕过尝试（`id # sftp-server`、`id; echo scp -t /tmp/x`、在共享账号自己的目录下放一个真的可执行文件冒充 `/tmp/fake/sftp-server` 并直接调用）全部被正确记录、未被误判为豁免。这次验证抓到了一个之前只做本地逻辑测试完全没发现的真实问题（见下一条），修完之后再跑了一遍确认。

## 已知取舍 / 限制

- **SFTP/SCP 和 rsync 都是完全直通、不审计**：`sshd` 对外部 sftp 子系统（`Subsystem sftp /path/to/sftp-server`）和 `rsync` 走 ssh 时的远程调用方式（实测确认固定是 `rsync --server ...` 开头）都跟 `ssh host "cmd"` 是同一种（`<shell> -c "<命令>"`），`session-shell` 识别到这两种调用会直接原样 `exec`，完全跳过录制逻辑——所以 SFTP/SCP/rsync 都能正常用，但传输的文件内容本身不会进审计日志。这是刻意选择，不是遗漏：这条 `-c` 命令路径本来就只记录 stdout/stderr、不记录 stdin，而文件传输的实际内容大多是走 stdin 上行的，就算不豁免、硬塞进 `tee` 管道，也录不到真正有用的内容（实测过 rsync 走 tee 管道协议本身不会坏，但 20MB 传输完日志只有十几 KB，全是协议握手/校验和之类的噪音）——干脆和 SFTP 一视同仁，直接豁免更干净。**踩过第四个坑，这个是端到端联调时才发现的，本地逻辑测试完全测不出来**：一直以为"SCP"都走 sftp 子系统，跑真实环境联调时才发现不对——OpenSSH 9.0（2022 年）之前的 `scp` 客户端（这次实测用的 Ubuntu 22.04 自带 OpenSSH 8.9p1，晚于这个日期的系统才默认切换成 sftp 协议）默认根本不用 sftp 子系统，而是走一套更老的协议，把远程端直接调用成 `scp -t <目标路径>`（上传）或 `scp -f <源路径>`（下载，`-r` 递归等参数会混在中间，如 `scp -r -t /tmp/dir`，均为真实抓包确认，不是猜的）——这是一次跟 `rsync --server ...` 结构上完全一样的、货真价实的 `-c "命令"` 调用，原来的豁免规则根本不认识它，会直接落进普通录制路径。实测过传输不会失败（跟 rsync 一样，`tee` 管道不会破坏这套简单的行协议），但会把整段加密数据当噪音录进日志，跟"SFTP/SCP 不审计传输内容"这句话自相矛盾。现在按同样的加固强度（不认 `$PATH`，在固定系统目录里找真正的 `scp` 二进制，`realpath` 解析后走 `is_root_locked` 校验整条属主链）把这两种调用形式也纳入豁免——校验时要求除最后一个路径参数外，其余每个词都得是单字母短参数或 `--`，且其中必须出现 `-t`/`-f` 之一，否则不算数、照常落进录制路径。实测过 `scp`/`scp -r`（含子目录）上传下载、5MB 二进制文件 md5 校验一致，且确认改完之后这两种调用不再产生任何审计记录。**踩过坑**：第一版实现没处理 SFTP 的识别逻辑，直接把 sftp 请求也塞进了录制管道，导致协议被破坏、`scp` 直接断连；更严重的是曾经误把还在正常用来做运维操作的账号（`ec2-user`）也切换成了这个 shell，当场把自己的 `scp` 堵死。如果需要审计"谁传了什么文件"，应该用 `sftp-server`/`rsync` 自带的日志能力（前者 `Subsystem sftp /path/to/sftp-server -l VERBOSE`，具体参数以 `man sftp-server`/`man rsyncd.conf` 为准），走 syslog，不是这个仓库现在做的事。
